...
class MMRI_I2P(nn.Module):
    ...
    def forward(self, lidar_feat, img_feat, img_metas, pts_metas, **kwargs):
        ...
        for b in range(batch_size):
            ...
            img_shape = img_metas[b]['batch_input_shape']
            ...
            mask = (mask & (xy_cams[..., 0:1] > -1.0) 
                 ...
                 & (xy_cams[..., 1:2] < 1.0))
            mask = torch.nan_to_num(mask)
            sampled_feat = F.grid_sample(img_feat[b],xy_cams.unsqueeze(-2)).squeeze(-1).permute(2,0,1) # https://gaussian37.github.io/dl-pytorch-snippets/
